def chia_het_cho_5(so_nhi_phan):
    ...
    if so_thap_phan % 5 == 0:
        ...
        return False
chuoi_so_hi_phan =  input("nhap chuoi so nhi phan (phan tach boi dau phay):")
so_nhi_phan_list = chuoi_so_hi_phan.split(',')
so_chia_het_cho_5 = [so for so in so_nhi_phan_list if chia_het_cho_5 (so)]
if len(so_chia_het_cho_5) > 0:
    ket_qua =  ','.join(so_chia_het_cho_5)
    print("cac so nhi phan chia het cho 5 laf:", ket_qua)
else:
